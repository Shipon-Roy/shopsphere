...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    await connectDB();
    const item = await BrandModel.findById(id).lean();
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    await connectDB();
    const item = await BrandModel.findByIdAndUpdate(id, body, { new: true, runValidators: true }).lean();
    if (!item) return NextResponse.json({ success: false, message: "Not found" }, { status: 404 });
    return NextResponse.json({ success: true, message: "Brand updated", data: item }, { status: 200 });
  } catch (e) { return err(e); }
}
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) return NextResponse.json({ success: false, message: "Invalid ID" }, { status: 400 });
    await connectDB();
    const item = await BrandModel.findByIdAndDelete(id);
    if (!item) return NextResponse.json({ success: false, message: "Not found" }, { status: 404 });
